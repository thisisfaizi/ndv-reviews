...
| T-UI2 | UI polish remainder (not done this pass): free `CriteriaPage`/`RequestsPage`/`ToolsPage` still use raw `.widefat`/`form-table` with no `.ndvr-card`; Pro `QandA\Moderation` has a few hand-typed hex colors off the token palette; `external_target_post` setting (read by `ExternalReviews::sync_all()`) has no input field anywhere — a pre-existing gap, not introduced this pass | free+pro/@fe | todo | screens migrated to `.ndvr-card`; no hardcoded off-palette colors; setting either gets a field or is removed |
