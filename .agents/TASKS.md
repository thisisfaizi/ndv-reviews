...
| T-QA2 | Finish the live-verification debt T-QA1 couldn't close: confirm the photo lightbox opens/closes/arrow-keys on a plain (non-Elementor-templated) product's native Reviews tab; confirm category-filtered + double-row marquee render on a plain page; run the full opt-in uninstall flow on a disposable install; re-trigger Pro's "+ Add Review" with "Approve immediately" checked to confirm the `RatingCache::recalc_product()` fatal-error fix actually resolves it live (only code-inspected against `Developer/Cli.php`, not re-run) | @qa | todo | lightbox click-through confirmed; marquee variants confirmed; uninstall dry-run confirmed on disposable install; manual review with approve-immediately succeeds with no fatal |
